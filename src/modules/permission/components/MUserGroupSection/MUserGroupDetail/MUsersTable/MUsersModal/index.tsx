...
export const MUsersModal = forwardRef<IMUsersModalRef, IMUsersModalProps>(
  ({ existingUsers = [], onAddUsers }, ref) => {
    ...
    const { data: _users = [], isFetching } = useQuery({
      queryKey: [
        "danh-sach-nhan-vien-trong-he-thong",
        params.code,
        params.name,
        open,
      ],
      enabled: open,
      queryFn: () => permissionsApi.getUsersInSystem(params),
    ...
    const [selection, setSelection] = useState<IUserInSystem[]>([]);
    //#endregion
    ...
    return (
      <Dialog open={open} onClose={onClose} maxWidth="lg">
        <Typography variant="dialog-title">
          Thêm nhân viên cho nhóm người dùng
        </Typography>
